/*
    ...
    The string has a length greater or equal to one and contains only letters from a to z. Examples: s="aaabbbbhaijjjm" printer_error(s) => "0/14" s="aaaxbbbbyyhwawiwjjjwwm" printer_error(s) => "8/22"
 */
...
namespace ConsoleApp2
{
    class Program
    {
        static string checkString = "abcdefghijklm";

        private static void printer_error(string controlString, out string errorRate)
        {
            int error = 0;
            foreach (char ch in controlString)
            {
            ...
            errorRate = $"{error}/{controlString.Length}";
        }
        static void Main(string[] args)
        {
            Console.WriteLine("~~~ Welcome to Box Labels Printer Interface ~~~");

            Console.Write("Enter the control string >> ");
            string controlString = Console.ReadLine();

            string result;
            printer_error(controlString, out result);
            Console.WriteLine("Error rate of a printer: " + result);
        }
    }
}
